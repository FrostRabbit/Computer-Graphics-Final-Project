...
void main() {
    ...
    if (dl.enable == 1) { 
        // ambient
        dAmbient = vec4(dl.lightColor * material.ambient, 1.0)*1.5;
    ...
    if (sl.enable == 1) {
        ...
        // ambient
        sAmbient = vec4(sl.lightColor * material.ambient, 1.0) * attenuation*0.1;
        ...
        if (dot(normalize(light_dir), normalize(sl.direction)) > sl.cutOff) {
            ...
            sDiffuse = vec4(sl.lightColor * material.diffuse, 1.0) * max(sdCoef, 0.0) * attenuation*2.0;
